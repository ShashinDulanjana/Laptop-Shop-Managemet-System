...
<head>
    ...
    <title>Edit Laptop - Modern Shop</title>
...
<body class="flex items-center justify-center min-h-screen py-10 px-4">
    ...
    <div class="w-full max-w-2xl">
        ...
        <div class="glass-card p-10 shadow-2xl">
            ...
            <form action="{{ route('laptops.update', $laptop->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                
                <div>
                    <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 ml-1">Brand</label>
                    <input type="text" name="brand" value="{{ $laptop->brand }}" class="input-field" required>
                </div>

                <div>
                    <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 ml-1">Model</label>
                    <input type="text" name="model" value="{{ $laptop->model }}" class="input-field" required>
                </div>

                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 ml-1">Price (LKR)</label>
                        <input type="number" name="price" min="1" value="{{ $laptop->price }}" class="input-field" required>
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 ml-1">Stock</label>
                        <input type="number" name="stock" min="0" value="{{ $laptop->stock }}" class="input-field" required>
                    </div>
                </div>

                <div>
                    <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 ml-1">Specifications</label>
                    <textarea name="specifications" rows="3" class="input-field">{{ $laptop->specifications }}</textarea>
                </div>

                <div class="p-6 bg-slate-900/50 rounded-2xl border border-slate-700 text-center">
                    <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4">Current Image</label>
                    @if($laptop->image)
                        <img src="{{ asset('images/' . $laptop->image) }}" class="h-32 mx-auto mb-4 rounded-lg shadow-lg object-contain">
                    @else
                        <p class="text-xs text-slate-500 mb-4">No image uploaded</p>
                    @endif
